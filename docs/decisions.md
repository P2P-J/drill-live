...
## 3. 깊이 단위 — **km, 1 tile = 1km**

- spec: km (지구 반지름 6,371km 모티프).
- 결정: spec 준수. UIScene에 km 표시. **1 tile (64px) = 1km**.
- 마이닝 속도 0.3초/타일 → 약 **3.3 km/s** 강하.
- 10시간 라이브 = 약 120,000km 깊이.
- 보스 도달 시점 (베이스 속도, 업그레이드/버프 없을 때):
  - Mega Mole (9,000km) — 약 45분
  - Crystal Golem (45,000km) — 약 3.8시간
  - Abyss Kraken (90,000km) — 약 7.5시간
  - Ancient Treant (450,000km) — 약 37시간 (단일 10h 라이브 불가)
  - Magma Dragon (950,000km) — 약 80시간 (단일 10h 라이브 불가)
- 후원 가속(TURBO ×3, OVERDRIVE ×5) 받으면 더 빨리 도달.
- 중력 계산은 깊이 단위와 무관하게 **픽셀 공간**에서 작동 (`GRAVITY_PX_S2 = 1250`). 시각적 자유낙하 가속만 영향.
